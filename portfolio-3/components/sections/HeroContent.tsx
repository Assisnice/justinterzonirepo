...
import { ContentComponentProps } from '../../types'; // Import ContentComponentProps

const HeroContent: React.FC<ContentComponentProps> = ({ openWindow }) => { // Destructure openWindow
  return (
    <div className="flex flex-col items-center justify-center h-full text-center p-4">
      ...
      <div className="mt-8 flex flex-col sm:flex-row justify-center space-y-3 sm:space-y-0 sm:space-x-3">
        <button 
          ...
          className="px-6 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-sky-600 hover:bg-sky-700 transition-transform transform hover:scale-105 shadow-md"
        >
          Explore My Journey
        </button>
        <button 
          onClick={() => openWindow?.('contact')}
          className="px-6 py-2 border border-sky-500 text-sm font-medium rounded-md text-sky-400 bg-transparent hover:bg-sky-500 hover:text-slate-900 transition-transform transform hover:scale-105 shadow-md"
        >
          Get In Touch
        </button>
      </div>
