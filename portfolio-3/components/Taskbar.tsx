...
import HomeIcon from './icons/HomeIcon'; // Placeholder, replace with actual icons
import UserIcon from './icons/UserIcon';
import CodeBracketIcon from './icons/CodeBracketIcon';
import BriefcaseIcon from './icons/BriefcaseIcon';
import EnvelopeIcon from './icons/EnvelopeIcon';
import CommandLineIcon from './icons/CommandLineIcon'; // Corrected import
...
const appIcons: Record<AppId, React.FC<React.SVGProps<SVGSVGElement>>> = {
  ...
  terminal: CommandLineIcon, // Now correctly references the imported icon
};

const Taskbar: React.FC<TaskbarProps> = ({ apps, openWindows, onOpenWindow, onFocusWindow, onMinimizeWindow }) => {
  ...
  const handleAppIconClick = (win: WindowInstance) => {
    if (win.isMinimized) {
      onOpenWindow(win.appId); // This will restore and focus
    } else {
      // Check if it's the top-most window by comparing zIndex
      const maxZIndex = Math.max(...openWindows.filter(w => !w.isMinimized).map(w => w.zIndex), 0);
      if (win.zIndex < maxZIndex) {
         onFocusWindow(win.id); // Bring to front if not already
      } else {
         onMinimizeWindow(win.id); // Minimize if already in front
      }
    }
  };
  ...
  return (
    <nav className="h-12 bg-os-title-bar/80 backdrop-blur-md text-slate-200 flex items-center justify-between px-2 shadow-taskbar border-t border-slate-700/50 z-[1000]">
      ...
      <div className="relative" ref={startMenuRef}>
        ...
        {isStartMenuOpen && (
          <div className="absolute bottom-full mb-2 left-0 w-64 bg-slate-800/90 backdrop-blur-md rounded-lg shadow-lg p-2 border border-slate-700">
            <div className="text-sm font-semibold px-2 py-1 text-sky-400">Justin Terzoni OS</div>
            {startMenuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className="w-full flex items-center text-left px-3 py-2 text-sm text-slate-200 hover:bg-sky-500/30 rounded-md transition-colors"
                >
          ...
          return (
            <button
              ...
              className={`h-full flex items-center px-3 py-1 rounded-md transition-colors ${
                !win.isMinimized && win.zIndex === Math.max(...openWindows.map(w => w.zIndex))
                  ? 'bg-sky-600/50 text-white'
                  : 'hover:bg-slate-700/70'
              } ${win.isMinimized ? 'border-b-2 border-sky-400' : ''}`}
              title={win.title}
            >
